...
Stage 1 as a whole is **not closed**. The repository otherwise proves a
substantial executable core: deterministic diagnostics, bounded expressions,
...
1. The canonical v1alpha1 reference remains procedural in the exact areas Stage
...
2. `plan --expanded` is deterministic and useful but not a complete projection
   of all normalized execution-affecting policy.
3. The repository lacks deterministic end-to-end completion evidence for the
   shell-orchestrated reference workflow and lacks a canonical gate-backed
   content-addressed benchmark.
4. The API-version separation is documented, but an explicit v1alpha1-to-new-
