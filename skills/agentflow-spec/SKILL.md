...
2. Select the simplest fit: a single implementation phase; implementation plus audit; a criterion workflow; a human-gated workflow; bookkeeping/completion; or, only when required, a dynamic criterion loop. For a new workflow that fits the concise core, prefer `agentflow.dev/v1alpha2`; its safe resume, checkpointing, phase evidence, and completion ordering are runtime-owned. Otherwise prefer `spec.defaults` with `spec.lifecycle.policy: safe-resume` in v1alpha1. Do not remove v1alpha1-only integrity, lineage, progress, human-gate, bookkeeping, or completion authority merely to migrate; follow `docs/guides/migrating-v1alpha1-to-v1alpha2.md`.
